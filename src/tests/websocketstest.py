...
baud_rate = 9600  # Baud rate of the GPS module
flag = True

# WebSocket server URL
server_uri = "wss://vehispot.onrender.com/vehicles/EstablishConnectionWithServer/"
...
async def getBusIdAndIpAddress():
    response = requests.get('https://{}/vehicles/getBusId/?deviceId=rasp0001')
    responseJson = response.json()
    return responseJson [ 'allotedBusId' ], responseJson [ 'ipAddress' ]


async def updateIpAddress(ipAddress: str):
    response = requests.post('https://{}/vehicles/updateDeviceIpAddress/',
                             data={"deviceId": "rasp0001", "ipAddress": ipAddress})
    responseJson = response.json()
    return responseJson
...
async def main():
    ...
    await connect_to_server_send_data("wss://vehispot.onrender.com/vehicles/EstablishConnectionWithServer/", busId)
